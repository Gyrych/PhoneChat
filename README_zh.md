...
- **移动端优化**：顶部收敛为“菜单 + 标题”单行，输入区底部吸附并考虑安全区，工具列横向滑动，抽屉改为全屏浮层，批量操作条顶部吸附，单手操作更友好。
...
2. **聊天工作区**
   ...
   - 输入区支持语音/附件占位、自动增高、token 进度条，且被固定在页面底部并自动留出安全区，不随滚动移动；`Enter/Ctrl+Enter` 发送，`Shift+Enter` 换行。
